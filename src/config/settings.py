"""Application configuration for CSPM Agent."""
...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="cspm-agent", description="Application name")
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5432/cspm_agent",
    ...
    opensearch_index: str = Field(default="cspm_agent_vectors", description="Index")
    ...
    mcp_server_name: str = Field(default="cspm-agent-mcp", description="MCP server name")

    a2a_discovery_url: str = Field(default="http://localhost:8500", description="A2A discovery URL")
    a2a_agent_id: str = Field(default="cspm-agent", description="A2A agent ID")
